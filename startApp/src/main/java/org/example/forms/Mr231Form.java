...
public class Mr231Form extends JDialog {
    ...
    Mr231StationType stationType = new Mr231StationType();

    public Mr231Form() {
        setContentPane(panel1);
        setModal(true);
        setTitle("Протокол МР-231");
        UpdateMessages();
    ...
    private void UpdateMessages() {
        ...
        try {
            ...
            rs = st.executeQuery("select * from tb_vhw");
            while (rs.next()) {
                WaterSpeedHeadingMessage vhw = new WaterSpeedHeadingMessage();
                vhw.setMsgRecTime(rs.getTimestamp(1));
                vhw.setCourse(rs.getDouble(2));
                vhw.setCourseAttr(rs.getString(3));
                vhw.setSpeed(rs.getDouble(4));
                vhw.setSpeedUnit(rs.getString(5));
    ...
    private void SendMessage() {
        Mr231Converter converter = stationType.createConverter();
        ...
        if (res.get(0).toString().contains("TrackedTargetMessage")) {
        ...
        } else if (res.get(0).toString().contains("RadarSystemData")) {
            ...
            WaterSpeedHeadingMessage msg = (WaterSpeedHeadingMessage) res.get(0);
            try {
                PreparedStatement statement = App.db.getConnection()
                        .prepareStatement("insert into tb_vhw (msgrectime, course, speed) " +
                                "values (?, ?, ?)");
                statement.setTimestamp(1, msg.getMsgRecTime());
                statement.setDouble(2, msg.getCourse());
                statement.setDouble(3, msg.getSpeed());
    ...
    private void GetMessageFromList() {
        ...
        if (msg.toString().contains("TrackedTargetMessage")) {
            ...
            textField1.setText("$RATTM," + ttm.getTargetNumber() + ","
                    ...
                    + status + ",,"
                    + ttm.getMsgTime() + ",A*");
        } else if (msg.toString().contains("RadarSystemData")) {
            ...
            WaterSpeedHeadingMessage vhw = (WaterSpeedHeadingMessage) msg;

            textField1.setText("$RAVHW," + vhw.getCourse() + ","
                    + vhw.getCourseAttr() + ",,,"
                    + vhw.getSpeed() + ","
                    + vhw.getSpeedUnit() + ",,*");
        }
